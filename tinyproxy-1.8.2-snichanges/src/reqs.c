...
void handle_connection (int fd)
{
	...
	if(1 == connptr->is_ssl)
	{
			...
			NULL != connptr->ssl_handshake)
		{
			...
			if(safe_write(connptr->server_fd, connptr->ssl_handshake, connptr->ssl_handshake_len) != connptr->ssl_handshake_len)
			{
				log_message(LOG_CONN, "Unable to write message to server");
				goto fail;
			}
